#!/usr/bin/env python3
"""
Convert the Word (.docx) chapter drafts stored under Archive/TI-MSP430G2553
into Quarto-flavored Markdown (.qmd) chapters + extracted images for the
book/ project.

This script is meant to be re-run whenever a chapter draft in Archive/ is
updated, or when a new chapter (e.g. CH06, CH07) is written up in Word and
needs to be brought into the Quarto book.
...
Usage:
    ...
    With no arguments, all configured chapters are (re)converted.
    Pass one or more chapter numbers (e.g. `1 3`) to only convert those.
"""
...
ARCHIVE = REPO_ROOT / "Archive" / "TI-MSP430G2553"
BOOK = REPO_ROOT / "book"
...
class Chapter:
    ...
    docx_dir: str
    docx_glob: str = "*.docx"
    references: str | None = None  # pre-cleaned reference list markdown, if any


# Known reference used verbatim by CH01 and CH05 in the original drafts.
FURBY_REFERENCE = (
    '1. P.-L. Ageneau, "Chaplier Fou," Aug. 25, 2023. [Online]. '
    "Available: <https://chapelierfou.org/blog/a-usb-furby.html>"
)

CHAPTERS = [
    Chapter(1, "introduction", "Introduction", "CH01 - Introduction", references=FURBY_REFERENCE),
    Chapter(2, "c-programming", "C Programming", "CH02 - C Programming"),
...
def reindent_braces(lines: list[str], width: int = 4) -> list[str]:
    ...
    The original Word paragraphs carry unreliable/inconsistent indentation
    (some nested lines are indented, sibling lines at the same nesting level
    are not), so instead of preserving it we recompute indentation from brace
    balance, which is the standard way to pretty-print C.
    """
    out = []
    depth = 0
    for raw in lines:
    ...
    while i < n:
        if lines[i].strip() == '::: {custom-style="Code"}':
            ...
            while i < n and lines[i].strip() != ":::":
                ...
                i += 1
            i += 1  # skip closing ':::'
            # Peek ahead past blank lines to see if another Code div follows.
            j = i
...
def demote_headings(text: str) -> str:
    ...
    original H1 section headings (Learning Objectives, ...) need to become H2
    to nest properly *under* the chapter. This also makes figure/table
    auto-numbering use the chapter number (e.g. "Figure 1.2") instead of a
    flat, book-wide heading counter.
    """
    out = []
    in_code = False
...
def convert_chapter(chapter: Chapter) -> None:
    ...
    with TemporaryDirectory() as tmp:
        ...
        # Copy final images (skip legacy .wmf/.emf sources; keep their .png).
