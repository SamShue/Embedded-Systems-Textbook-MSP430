...
|----|----|
| `long` | increases the memory allocated to a data type, allowing it to hold larger values |
| `short` | decreases the memory allocated to a data type, allowing it to hold smaller values while using less memory |
| `unsigned` | treats a variable as holding only non-negative values, doubling the range of positive values it can represent |
| `signed` | treats a variable as being able to hold both positive and negative values (the default for most data types) |
...
|----|----|
| `const` | specifies that a variable's value is constant and cannot be changed after it is assigned |
| `static` | allows a variable to retain its value between function calls, even after going out of scope |
| `volatile` | forces the compiler to build every instruction involving the variable, preventing it from being optimized out |
...
![Example section of the text region containing compiled instructions.](../images/ch02/image4.png){#fig-ch02-2-4}
...
![Example section of the text region containing compiled instructions.](../images/ch02/image5.png){#fig-ch02-2-5}
...
![Example C code and the condition of the stack up to the indicated break point. “X” indicates bit values that have been uninitialized.](../images/ch02/image6.png){#fig-ch02-2-6}
...
![Example C code and the condition of the stack up to the indicated break point. “X” indicates bit values that have been uninitialized.](../images/ch02/image7.png){#fig-ch02-2-7}
...
![The Cross-Compilation Process](../images/ch02/image9.png){#fig-ch02-2-9}
...
When developing for MSP430 microcontrollers, or any TI microcontroller, Code Composer Studio (CCS) is the IDE provided by TI to support their products. CCS contains the all the tools listed in the previous section, plus a few more, such as a resource explorer, where example programs can be imported into your CCS environment to quickly get started developing code.

#### Code Composer UI Overview

Let’s take a look at the user interface (UI) of the CCS environment. We’ll start by highlighting the large section in the center of the window, the text editor. The locations of these windows shown here are the defaults, but they can be moved around per the user’s preferences. The text editor provides syntax highlighting and a space to edit the code. It also can support the addition of breakpoints, which when in debugging mode will allow the processor to stop execution at the line of code marked with a breakpoint and allow the user to inspect things such as memory content and variable values.
...
![The Code Composer Studio IDE](../images/ch02/image10.png){#fig-ch02-2-10}
...
![A screenshot of a computer Description automatically generated](../images/ch02/image11.png){width="3.4303171478565178in" height="2.572911198600175in"}

….

![Where to find MSP430G2553 Demo Code in the Resource Explorerv](../images/ch02/image12.png){#fig-ch02-2-11}
