...
### Is It Really Necessary to Embed a Computer?

In many embedded systems applications, an entire computer is used in place of what could otherwise be a custom analog circuit. One example of a device that utilizes a microcontroller for cost benefits and flexibility is a thermostat.

Traditionally, thermostats used analog circuits to control the temperature, employing components like resistors, capacitors, and relays. @fig-ch01-thermostat-analog shows an example of one such analog thermostat control board. This board reads the temperature settings from switches or potentiometers set by the installer or homeowner, and switches relays on and off to turn a furnace or air conditioner on and off – all without a single line of code.

![An analog thermostat control board. Relays (the black components) and other discrete components are wired together to control heating and cooling equipment.](../images/ch01/thermostat_analog.jpeg){#fig-ch01-thermostat-analog}

Many modern thermostats now utilize microcontrollers for enhanced functionality and cost-effectiveness, as shown in @fig-ch01-thermostat-mcu.

![A microcontroller-based thermostat control board. The microcontroller (the black IC to the right of the 7-segment displays) reads button presses, drives the display, and switches the same kind of relay used in the analog design.](../images/ch01/thermostat_mcu.jpeg){#fig-ch01-thermostat-mcu}

**Flexibility**

-   A microcontroller allows for programmability, enabling the thermostat to have various features, such as scheduling different temperatures throughout the day or displaying the current temperature on a digital display.

-   These features can be updated or customized through firmware updates, without having to redesign the analog control circuitry.

**Cost-effectiveness**

-   Versatility and mass production benefits of microcontrollers often result in lower overall costs.

-   Microcontrollers can handle multiple functions, reducing the need for additional discrete components. Instead of designing separate custom circuits for scheduling, button input, and display functionality, a single microcontroller can be programmed to handle all of these behaviors.

**Diagnostic capabilities**

-   Microcontrollers can monitor various parameters and detect faults or inefficiencies in the system.

-   This allows for proactive maintenance and troubleshooting, such as alerting a homeowner that a filter needs to be changed or that a sensor is reading an unexpected value.

Notice that both thermostat designs in @fig-ch01-thermostat-analog and @fig-ch01-thermostat-mcu still rely on the same kind of relay to physically switch the furnace or air conditioner on and off. The microcontroller does not replace this hardware, but rather replaces the analog logic that used to decide *when* to activate it, while also adding capabilities – such as scheduling, a digital display, and diagnostics – that would be very difficult to implement using discrete analog components alone.
...
![Blink an LED using a relay, capacitor, and resistors.](../images/ch01/analog_blink_circuit.png){#fig-ch01-economics-1}

@fig-ch01-economics-1 shows a design that blinks an LED using only a relay, a capacitor, and a couple of resistors – no microcontroller required. Pricing each of these discrete components individually gives us the following:
...
![Blink an LED using a microcontroller and a resistor.](../images/ch01/mcu_blink_circuit.png){#fig-ch01-economics-2}

Now let’s consider the same task using a microcontroller, as shown in @fig-ch01-economics-2. Pricing these components individually gives us the following:
...
![An overview of the MSP-EXP430G2ET Launchpad development board](../images/ch01/image30.png){#fig-ch01-1-3}
...
![How the legs of the microcontroller connect to the header pins of the development board.](../images/ch01/image40.png){#fig-ch01-1-4}
